"""Verify every approved Audius seed through the external connector.

The script reads and discards returned audio bytes. It proves provider fetch and
decoder success, not that a person heard audio from a physical speaker.
"""
...
def verify(
    *, base_url: str, seed_path: Path, timeout_seconds: float
    ...
    if not isinstance(tracks, list) or not tracks:
        raise ValueError("seed catalog has no tracks")
    ...
    return {
        "component": "AUDIUS_VERIFIED_SEED_PREVIEW",
        "provider": "AUDIUS",
        "network_scope": "INTERNET",
